...
const IndexCard: React.FC<IndexCardProps> = (indexProps) => {
    const { tokens, contract, name, account, ethereum, id, zap, creator } = indexProps;
    ...
    return (
        <ICard>
            ...
            <CardBody>
                ...
                <Flex alignItems='center' justifyContent='space-between'>
                        <Text  fontSize="16px" bold style={{ display: 'flex', alignItems: 'center'}}>Creator : </Text>
                        <Text color="primary" fontSize="16px" bold style={{ display: 'flex', alignItems: 'center'}}>{creator}</Text>
                </Flex>
            </CardBody>
            <Actions>
                {account ? (<>
